...
class GSRegresser(nn.Module):
    def __init__(self, cfg, rgb_dim=3, depth_dim=1, norm_fn='group'):
        ...
        print("self.rgb_dims", self.rgb_dims)
        print("self.depth_dims", self.depth_dims)
        print("self.decoder_dims", self.decoder_dims)
        print("self.head_dim", self.head_dim)
        # print(f"in_channe = {depth_dim}, encoder_dim = {self.depth_dims}")
        self.depth_encoder = UnetExtractor(in_channel=depth_dim, encoder_dim=self.depth_dims)
        self.feature_encoder = UnetExtractor(in_channel=rgb_dim, encoder_dim=self.rgb_dims)
        # self.pc_encoder = UnetExtractor(in_channel=rgb_dim, encoder_dim=self.depth_dims)
        #============================decoder original==============================
        ...
        # self.out_conv = nn.Conv2d(self.decoder_dims[0], self.head_dim, kernel_size=3, padding=1)
    ...
    def forward(self, img, depth, img_feat):
        ...
        # depth_interpolated = F.interpolate(1/(depth+1e-8), size=(H // 4, W // 4), mode='bilinear', align_corners=False)
        depth_feat1, depth_feat2, depth_feat3 = self.depth_encoder(depth)
        img_feat1, img_feat2, img_feat3  = self.feature_encoder(img)
        # img_feat1, img_feat2, img_feat3 = img_feat
        # print("depth_feat1 max, min", depth_feat1.max(), depth_feat1.min())
        # print("depth_feat2 max, min", depth_feat2.max(), depth_feat2.min())
        # print("depth_feat3 max, min", depth_feat3.max(), depth_feat3.min())
        # print("img_feat1 max, min", img_feat1.max(), img_feat1.min())
        # print("img_feat2 max, min", img_feat2.max(), img_feat2.min())
        # print("img_feat3 max, min", img_feat3.max(), img_feat3.min())
        # print("depth_feat1, depth_feat2, depth_feat3 ", depth_feat1.shape, depth_feat2.shape, depth_feat3.shape)
        # print("img_feat1, img_feat2, img_feat3 shape", img_feat1.shape, img_feat2.shape, img_feat3.shape)
        # Resize các feature và image về cùng kích thước của depth
        # img_resized = nn.functional.interpolate(img, size=(180, 320), mode='bilinear', align_corners=False)
        # img_feat_resized_1 = nn.functional.interpolate(img_feat, size=(192, 320), mode='bilinear', align_corners=False)
        # img_feat_resized_2 = img_feat
        # img_feat_resized_3 = nn.functional.interpolate(img_feat, size=(48, 80), mode='bilinear', align_corners=False)
        ...
        # print("feat1, feat2, feat3 shape", feat1.shape, feat2.shape, feat3.shape)
        # exit()
        #============================3 feature================================
        ...
        # scale_out = torch.clamp_min(self.scale_head(out), 0.01)
        # opacity head
