...
class VolumeDecoder(nn.Module):
    ...
    def transform_point_cloud_to_world(self, point_cloud_camera, cam2world):
        ...
        Args:
            ...
            extrinsic (torch.Tensor): Extrinsic matrix of shape [B, 4, 4].
    ...
    def prepare_gs_attribute(self, inputs, rot_map, scale_map, depth_map, opacity_map, point_cloud, is_train = None, index = 0):
        # prepare gaussian
        pc = {}
        bs = inputs["color_aug", 0, 0].shape[0]
        ...
        # else:
        pc['colors_precomp'] = inputs["color_aug", 0, 0]
    ...
    def get_splatting_rendering(self, K, C2W, pc, inputs, flow_index = (0, 0)):
        ...
        for j in range (C2W.shape[0]):
            ...
            # print("pc_j['active_sh_degree']", pc_j['active_sh_degree'])
            ...
            # for key, value in viewpoint_camera.items():
            #     print(f"Key: {key}")
            #     print(f"Shape: {value.shape if isinstance(value, torch.Tensor) or isinstance(value, np.ndarray) else 'Not an array/tensor'}")
            #     print(f"Value: {value}\n")
            render_pkg = splatting_render(viewpoint_camera, pc_j, opt = self.opt)
            # print("render_pkg['render'] shape", render_pkg['render'].shape)
            # depth_i = render_pkg['depth'].view(self.opt.render_h, self.opt.render_w).unsqueeze(0)
            # depth_i =
            rgb_marched_i = render_pkg['render'].unsqueeze(0)
            # print("depth_i shape", depth_i.shape)
            # print("rgb_marched_i shape", rgb_marched_i.shape)
